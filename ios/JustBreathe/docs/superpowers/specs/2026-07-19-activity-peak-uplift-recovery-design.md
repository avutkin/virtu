...
lives in `Metrics/` and is covered by `JustBreatheTests`, while views are not). It
...
### Tests — `JustBreatheTests/ActivityMetricStatsTests.swift`
...
| Modify | `JustBreathe.xcodeproj/project.pbxproj` — register the 2 new files |
