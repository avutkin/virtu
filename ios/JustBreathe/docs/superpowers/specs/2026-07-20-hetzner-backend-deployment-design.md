# JustBreathe Backend — Hetzner Deployment Design
...
- A `justbreathe-api.service` systemd unit (to be superseded by Docker Compose).
...
- Compose file, Caddyfile, and a git-ignored `.env` live on the server (repo checked out to `/opt/justbreathe` or `~/just-breathe`).
...
- **Nightly `pg_dump`** (cron or a small compose sidecar) to `/opt/justbreathe/backups`, retained ~7 days.
