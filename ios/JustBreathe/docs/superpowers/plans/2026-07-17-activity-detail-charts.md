...
```bash
cd /Users/alexutkin/ios/JustBreathe/UI/Activities
...
In `JustBreathe.xcodeproj/project.pbxproj`, make these 4 exact replacements (same IDs `F146`/`A146`, just renamed path/comment — this is a rename, not a new file, so no new IDs):
...
```bash
cd /Users/alexutkin/ios && git add JustBreathe/UI/Activities/ActivityMetricsGrid.swift JustBreathe/UI/Activities/ActivityMetricsCard.swift JustBreathe.xcodeproj/project.pbxproj
...
Write `ios/JustBreathe/UI/Activities/ActivityWindowChart.swift`:
...
In `JustBreathe.xcodeproj/project.pbxproj`, make these 4 additions (new IDs `F147`/`A147` — confirmed unused by `grep -n "F147\|A147" JustBreathe.xcodeproj/project.pbxproj`, one past the `F146`/`A146` pair):
...
```bash
cd /Users/alexutkin/ios && git add JustBreathe/UI/Activities/ActivityWindowChart.swift JustBreathe.xcodeproj/project.pbxproj
...
xcodebuild -project JustBreathe.xcodeproj -scheme JustBreathe -destination 'platform=iOS Simulator,name=iPhone 17 Pro' -derivedDataPath /tmp/jb-build-detail build 2>&1 | tail -5
xcrun simctl install "iPhone 17 Pro" /tmp/jb-build-detail/Build/Products/Debug-iphonesimulator/JustBreathe.app
xcrun simctl launch "iPhone 17 Pro" com.alexutkin.justbreathe
```
