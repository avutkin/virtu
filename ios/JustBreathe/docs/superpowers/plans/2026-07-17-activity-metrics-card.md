...
- No unit-test target covers SwiftUI view/model files in this codebase (`JustBreatheTests` only covers BLE parsing and metrics compute logic) — verification for every task is **build success** (`xcodebuild build`) plus, where noted, a manual Simulator check.
...
```bash
cd /Users/alexutkin/ios && git add JustBreathe/Models/ActivityLog.swift
...
Write `ios/JustBreathe/UI/Design/MetricTile.swift`:
...
In `ios/JustBreathe/UI/Live/LiveView.swift`, find and delete this entire block (it currently sits directly after `MetricsTableView`, just before `// MARK: - Preview`):
...
In `JustBreathe.xcodeproj/project.pbxproj`, make these 4 additions (new unique IDs `F145`/`A145` — confirmed not already used by `grep -oE "F1[0-9]+|A1[0-9]+" JustBreathe.xcodeproj/project.pbxproj`, current highest is `F144`/`A144`):
...
```bash
cd /Users/alexutkin/ios && git add JustBreathe/UI/Design/MetricTile.swift JustBreathe/UI/Live/LiveView.swift JustBreathe.xcodeproj/project.pbxproj
...
Write `ios/JustBreathe/UI/Activities/ActivityMetricsCard.swift`:
...
In `JustBreathe.xcodeproj/project.pbxproj`, make these 4 additions (new IDs `F146`/`A146` — one past the `F145`/`A145` pair added in Task 2):
...
```bash
cd /Users/alexutkin/ios && git add JustBreathe/UI/Activities/ActivityMetricsCard.swift JustBreathe.xcodeproj/project.pbxproj
...
xcodebuild -project JustBreathe.xcodeproj -scheme JustBreathe -destination 'platform=iOS Simulator,name=iPhone 17 Pro' -derivedDataPath /tmp/jb-build-t2t4 build 2>&1 | tail -5
xcrun simctl install "iPhone 17 Pro" /tmp/jb-build-t2t4/Build/Products/Debug-iphonesimulator/JustBreathe.app
BUNDLE_ID=$(/usr/libexec/PlistBuddy -c "Print :CFBundleIdentifier" /tmp/jb-build-t2t4/Build/Products/Debug-iphonesimulator/JustBreathe.app/Info.plist)
