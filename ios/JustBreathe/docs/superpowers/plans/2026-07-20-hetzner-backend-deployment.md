# JustBreathe Backend — Hetzner Deployment Implementation Plan
...
Run: `cd ~/server && docker build -t justbreathe-api:local .`
Expected: build succeeds, ends with `naming to docker.io/library/justbreathe-api:local`.
...
- Delete: `~/server/justbreathe-api.service` (superseded by Compose)
...
services:
  db:
    ...
    environment:
      POSTGRES_USER: justbreathe
      POSTGRES_PASSWORD: ${POSTGRES_PASSWORD}
      POSTGRES_DB: justbreathe
    volumes:
      - pgdata:/var/lib/postgresql/data
    healthcheck:
      test: ["CMD-SHELL", "pg_isready -U justbreathe"]
      interval: 10s
      timeout: 5s
      retries: 5
  ...
  api:
    ...
    environment:
      DATABASE_URL: postgresql://justbreathe:${POSTGRES_PASSWORD}@db:5432/justbreathe
...
Run: `cd /Users/alexutkin && git rm server/justbreathe-api.service`
...
Or CLI (`brew install hcloud`, then `hcloud context create justbreathe` with an API token):
```bash
hcloud server create --name justbreathe-api --type cax11 \
  --image ubuntu-24.04 --location nbg1 --ssh-key <your-key-name>
```

- [ ] **Step 2: Record the public IP**

Run: `hcloud server ip justbreathe-api` (or read it in the console).
...
Console: Firewalls → Create → inbound allow TCP 22, 80, 443 from anywhere → apply to `justbreathe-api`.
Or CLI:
```bash
...
hcloud firewall apply-to-resource web --type server --server justbreathe-api
```
...
The app's `DATABASE_URL` uses host `db` (the compose service name), not `localhost`. Confirm `docker-compose.yml` sets `DATABASE_URL=postgresql://justbreathe:...@db:5432/justbreathe` (it does, from Task 6).
...
mkdir -p backups
STAMP=$(date +%F-%H%M)
docker compose exec -T db pg_dump -U justbreathe justbreathe | gzip > "backups/justbreathe-$STAMP.sql.gz"
# Retain 7 days
...
Expected: a `justbreathe-<stamp>.sql.gz` file exists and is non-empty.
...
```bash
hcloud server create-image --type snapshot --description "justbreathe weekly" justbreathe-api
```
...
**Files:**
- Create: `~/ios/JustBreathe/App/ServerConfig.swift`
- Modify: `~/ios/JustBreathe/App/AppEnvironment.swift:74-75,107-108` (default server URL)
- Modify: `~/ios/JustBreathe/Sync/APIClient.swift:139-144` (`request` helper — add auth header)
- Modify: `~/ios/JustBreathe/Sync/SyncService.swift:60-70` (`connect` — add `?token=`)
...
In `~/ios/JustBreathe/App/AppEnvironment.swift`, replace both `"http://localhost:8000"` fallbacks (lines ~74-75 and ~107-108) with `ServerConfig.defaultBaseURLString`. Example at line 74-75:
...
In `~/ios/JustBreathe/Sync/APIClient.swift`, in the private `request(path:method:)` helper, add the auth header:
...
In `~/ios/JustBreathe/Sync/SyncService.swift`, in `connect(userID:)`, add the token query item before building the URL:
...
```bash
cd /Users/alexutkin
git add ios/JustBreathe/App/ServerConfig.swift ios/JustBreathe/App/AppEnvironment.swift \
        ios/JustBreathe/Sync/APIClient.swift ios/JustBreathe/Sync/SyncService.swift
