...
**Tech Stack:** Swift 5 / SwiftUI, Swift Charts, XCTest (`JustBreatheTests`), Xcode project with explicit `PBXFileReference`/`PBXGroup` entries (new files registered in `project.pbxproj` by hand).

## Global Constraints

- Compute logic is unit-tested (`JustBreatheTests`); SwiftUI views are not — matches codebase convention. Verification: build success + the new compute tests passing, plus a manual/visual check where noted.
- Build command:
  ```bash
...
- Test command (whole suite):
  ```bash
  cd /Users/alexutkin/ios && xcodebuild test -project JustBreathe.xcodeproj -scheme JustBreathe -destination 'platform=iOS Simulator,name=iPhone 17 Pro' 2>&1 | grep -E "error:|failed \(|Executed"
  ```
...
- Create: `JustBreatheTests/ActivityMetricStatsTests.swift`
...
- [ ] **Step 2: Write `JustBreatheTests/ActivityMetricStatsTests.swift`**
...
- [ ] **Step 3: Register both files in `JustBreathe.xcodeproj/project.pbxproj`**
...
Addition F — add `FT07` to the `JustBreatheTests` group (`GTESTS`), after the `FT06 /* PayloadBuilderTests.swift */,` child:
Find:
```
...
```bash
cd /Users/alexutkin/ios && xcodebuild test -project JustBreathe.xcodeproj -scheme JustBreathe -destination 'platform=iOS Simulator,name=iPhone 17 Pro' -only-testing:JustBreatheTests/ActivityMetricStatsTests 2>&1 | grep -E "error:|failed \(|passed \(|Executed"
```
...
```bash
cd /Users/alexutkin/ios && git add JustBreathe/Metrics/ActivityMetricStats.swift JustBreatheTests/ActivityMetricStatsTests.swift JustBreathe.xcodeproj/project.pbxproj
git commit -m "feat(metrics): add ActivityMetricStats for benefit-aware peak/uplift/recovery"
```
(The test file lives at `ios/JustBreatheTests/ActivityMetricStatsTests.swift` — a sibling of `JustBreathe/`, alongside `PayloadBuilderTests.swift` etc. — NOT nested under `JustBreathe/`.)
...
```bash
cd /Users/alexutkin/ios && git add JustBreathe/UI/Design/MetricTile.swift
...
```bash
cd /Users/alexutkin/ios && xcodebuild test -project JustBreathe.xcodeproj -scheme JustBreathe -destination 'platform=iOS Simulator,name=iPhone 17 Pro' 2>&1 | grep -E "failed \(|Executed"
```
