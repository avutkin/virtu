...
**Tech Stack:** Swift 5 / SwiftUI, Swift Charts, XCTest (`PulsarTests`), Xcode project with explicit `PBXFileReference`/`PBXGroup` entries (new files registered in `project.pbxproj` by hand).

## Global Constraints

- Compute logic is unit-tested (`PulsarTests`); SwiftUI views are not — matches codebase convention. Verification: build success + the new compute tests passing, plus a manual/visual check where noted.
- Build command:
  ```bash
  cd /Users/alexutkin/ios && xcodebuild build -project Pulsar.xcodeproj -scheme Pulsar -destination 'platform=iOS Simulator,name=iPhone 17 Pro' 2>&1 | tail -40
  ```
  Expected final line: `** BUILD SUCCEEDED **`
- Test command (whole suite):
  ```bash
  cd /Users/alexutkin/ios && xcodebuild test -project Pulsar.xcodeproj -scheme Pulsar -destination 'platform=iOS Simulator,name=iPhone 17 Pro' 2>&1 | grep -E "error:|failed \(|Executed"
  ```
...
- Create: `PulsarTests/ActivityMetricStatsTests.swift`
- Modify: `Pulsar.xcodeproj/project.pbxproj`
...
- [ ] **Step 2: Write `PulsarTests/ActivityMetricStatsTests.swift`**
...
- [ ] **Step 3: Register both files in `Pulsar.xcodeproj/project.pbxproj`**
...
Addition F — add `FT07` to the `PulsarTests` group (`GTESTS`), after the `FT06 /* PayloadBuilderTests.swift */,` child:
Find:
```
...
```bash
cd /Users/alexutkin/ios && xcodebuild test -project Pulsar.xcodeproj -scheme Pulsar -destination 'platform=iOS Simulator,name=iPhone 17 Pro' -only-testing:PulsarTests/ActivityMetricStatsTests 2>&1 | grep -E "error:|failed \(|passed \(|Executed"
```
...
```bash
cd /Users/alexutkin/ios && git add Pulsar/Metrics/ActivityMetricStats.swift PulsarTests/ActivityMetricStatsTests.swift Pulsar.xcodeproj/project.pbxproj
git commit -m "feat(metrics): add ActivityMetricStats for benefit-aware peak/uplift/recovery"
```
(The test file lives at `ios/PulsarTests/ActivityMetricStatsTests.swift` — a sibling of `Pulsar/`, alongside `PayloadBuilderTests.swift` etc. — NOT nested under `Pulsar/`.)
...
```bash
cd /Users/alexutkin/ios && git add Pulsar/UI/Design/MetricTile.swift
...
```bash
cd /Users/alexutkin/ios && xcodebuild test -project Pulsar.xcodeproj -scheme Pulsar -destination 'platform=iOS Simulator,name=iPhone 17 Pro' 2>&1 | grep -E "failed \(|Executed"
```
