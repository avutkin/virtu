...
- No unit-test target covers SwiftUI view/model files in this codebase (`PulsarTests` only covers BLE parsing and metrics compute logic) — verification for every task is **build success** (`xcodebuild build`) plus, where noted, a manual Simulator check.
...
```bash
cd /Users/alexutkin/ios && git add Pulsar/Models/ActivityLog.swift
...
Write `ios/Pulsar/UI/Design/MetricTile.swift`:
...
In `ios/Pulsar/UI/Live/LiveView.swift`, find and delete this entire block (it currently sits directly after `MetricsTableView`, just before `// MARK: - Preview`):
...
In `Pulsar.xcodeproj/project.pbxproj`, make these 4 additions (new unique IDs `F145`/`A145` — confirmed not already used by `grep -oE "F1[0-9]+|A1[0-9]+" Pulsar.xcodeproj/project.pbxproj`, current highest is `F144`/`A144`):
...
```bash
cd /Users/alexutkin/ios && git add Pulsar/UI/Design/MetricTile.swift Pulsar/UI/Live/LiveView.swift Pulsar.xcodeproj/project.pbxproj
...
Write `ios/Pulsar/UI/Activities/ActivityMetricsCard.swift`:
...
In `Pulsar.xcodeproj/project.pbxproj`, make these 4 additions (new IDs `F146`/`A146` — one past the `F145`/`A145` pair added in Task 2):
...
```bash
cd /Users/alexutkin/ios && git add Pulsar/UI/Activities/ActivityMetricsCard.swift Pulsar.xcodeproj/project.pbxproj
...
xcodebuild -project Pulsar.xcodeproj -scheme Pulsar -destination 'platform=iOS Simulator,name=iPhone 17 Pro' -derivedDataPath /tmp/jb-build-t2t4 build 2>&1 | tail -5
xcrun simctl install "iPhone 17 Pro" /tmp/jb-build-t2t4/Build/Products/Debug-iphonesimulator/Pulsar.app
BUNDLE_ID=$(/usr/libexec/PlistBuddy -c "Print :CFBundleIdentifier" /tmp/jb-build-t2t4/Build/Products/Debug-iphonesimulator/Pulsar.app/Info.plist)
...
```bash
cd /Users/alexutkin/ios && git add Pulsar/UI/Activities/ActivitiesView.swift
