...
- Modify: `App/PulsarApp.swift:67-86`
...
In `App/PulsarApp.swift`, `ContentView.body` currently ends with:
...
```bash
cd /Users/alexutkin/ios && git add Pulsar/App/AppEnvironment.swift Pulsar/App/PulsarApp.swift
...
```bash
cd /Users/alexutkin/ios/Pulsar/UI
...
In `Pulsar.xcodeproj/project.pbxproj`, there are 4 places referencing the old path/name. Make these exact replacements:
...
```bash
cd /Users/alexutkin/ios && git add -A Pulsar/UI/Activities Pulsar/UI/Actions Pulsar.xcodeproj/project.pbxproj
...
xcodebuild -project Pulsar.xcodeproj -scheme Pulsar -destination 'platform=iOS Simulator,name=iPhone 17 Pro' -derivedDataPath /tmp/jb-build build 2>&1 | tail -5
xcrun simctl install "iPhone 17 Pro" /tmp/jb-build/Build/Products/Debug-iphonesimulator/Pulsar.app
BUNDLE_ID=$(/usr/libexec/PlistBuddy -c "Print :CFBundleIdentifier" /tmp/jb-build/Build/Products/Debug-iphonesimulator/Pulsar.app/Info.plist)
...
```bash
cd /Users/alexutkin/ios && git add Pulsar/UI/Activities/ActivitiesView.swift Pulsar/App/PulsarApp.swift
...
```bash
cd /Users/alexutkin/ios && xcodebuild clean build -project Pulsar.xcodeproj -scheme Pulsar -destination 'platform=iOS Simulator,name=iPhone 17 Pro' 2>&1 | tail -40
```
Expected: `** BUILD SUCCEEDED **`
