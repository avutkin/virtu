...
```bash
cd /Users/alexutkin/ios/Pulsar/UI/Activities
...
In `Pulsar.xcodeproj/project.pbxproj`, make these 4 exact replacements (same IDs `F146`/`A146`, just renamed path/comment — this is a rename, not a new file, so no new IDs):
...
```bash
cd /Users/alexutkin/ios && git add Pulsar/UI/Activities/ActivityMetricsGrid.swift Pulsar/UI/Activities/ActivityMetricsCard.swift Pulsar.xcodeproj/project.pbxproj
...
Write `ios/Pulsar/UI/Activities/ActivityWindowChart.swift`:
...
In `Pulsar.xcodeproj/project.pbxproj`, make these 4 additions (new IDs `F147`/`A147` — confirmed unused by `grep -n "F147\|A147" Pulsar.xcodeproj/project.pbxproj`, one past the `F146`/`A146` pair):
...
```bash
cd /Users/alexutkin/ios && git add Pulsar/UI/Activities/ActivityWindowChart.swift Pulsar.xcodeproj/project.pbxproj
...
xcodebuild -project Pulsar.xcodeproj -scheme Pulsar -destination 'platform=iOS Simulator,name=iPhone 17 Pro' -derivedDataPath /tmp/jb-build-detail build 2>&1 | tail -5
xcrun simctl install "iPhone 17 Pro" /tmp/jb-build-detail/Build/Products/Debug-iphonesimulator/Pulsar.app
xcrun simctl launch "iPhone 17 Pro" com.alexutkin.pulsar
```
