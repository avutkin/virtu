...
`ContentView` (in `PulsarApp.swift`) observes it:
...
| Modify | `App/PulsarApp.swift` — `AppTab.actions` → `.activities`, tab label, `ActionsView()` → `ActivitiesView()`, `.onChange(of: env.pendingTabRequest)` |
