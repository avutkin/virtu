...
lives in `Metrics/` and is covered by `PulsarTests`, while views are not). It
...
### Tests — `PulsarTests/ActivityMetricStatsTests.swift`
...
| Modify | `Pulsar.xcodeproj/project.pbxproj` — register the 2 new files |
