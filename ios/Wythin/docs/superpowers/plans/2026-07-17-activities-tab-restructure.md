...
- Modify: `App/WythinApp.swift:67-86`
...
In `App/WythinApp.swift`, `ContentView.body` currently ends with:
...
```bash
cd /Users/alexutkin/ios && git add Wythin/App/AppEnvironment.swift Wythin/App/WythinApp.swift
...
```bash
cd /Users/alexutkin/ios/Wythin/UI
...
In `Wythin.xcodeproj/project.pbxproj`, there are 4 places referencing the old path/name. Make these exact replacements:
...
```bash
cd /Users/alexutkin/ios && git add -A Wythin/UI/Activities Wythin/UI/Actions Wythin.xcodeproj/project.pbxproj
...
xcodebuild -project Wythin.xcodeproj -scheme Wythin -destination 'platform=iOS Simulator,name=iPhone 17 Pro' -derivedDataPath /tmp/jb-build build 2>&1 | tail -5
xcrun simctl install "iPhone 17 Pro" /tmp/jb-build/Build/Products/Debug-iphonesimulator/Wythin.app
BUNDLE_ID=$(/usr/libexec/PlistBuddy -c "Print :CFBundleIdentifier" /tmp/jb-build/Build/Products/Debug-iphonesimulator/Wythin.app/Info.plist)
...
```bash
cd /Users/alexutkin/ios && git add Wythin/UI/Activities/ActivitiesView.swift Wythin/App/WythinApp.swift
...
```bash
cd /Users/alexutkin/ios && xcodebuild clean build -project Wythin.xcodeproj -scheme Wythin -destination 'platform=iOS Simulator,name=iPhone 17 Pro' 2>&1 | tail -40
```
Expected: `** BUILD SUCCEEDED **`
