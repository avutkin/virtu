# Wythin Backend — Hetzner Deployment Implementation Plan
...
Run: `cd ~/server && docker build -t wythin-api:local .`
Expected: build succeeds, ends with `naming to docker.io/library/wythin-api:local`.
...
- Delete: `~/server/wythin-api.service` (superseded by Compose)
...
services:
  db:
    ...
    environment:
      POSTGRES_USER: wythin
      POSTGRES_PASSWORD: ${POSTGRES_PASSWORD}
      POSTGRES_DB: wythin
    volumes:
      - pgdata:/var/lib/postgresql/data
    healthcheck:
      test: ["CMD-SHELL", "pg_isready -U wythin"]
      interval: 10s
      timeout: 5s
      retries: 5
  ...
  api:
    ...
    environment:
      DATABASE_URL: postgresql://wythin:${POSTGRES_PASSWORD}@db:5432/wythin
...
Run: `cd /Users/alexutkin && git rm server/wythin-api.service`
...
Or CLI (`brew install hcloud`, then `hcloud context create wythin` with an API token):
```bash
hcloud server create --name wythin-api --type cax11 \
  --image ubuntu-24.04 --location nbg1 --ssh-key <your-key-name>
```

- [ ] **Step 2: Record the public IP**

Run: `hcloud server ip wythin-api` (or read it in the console).
...
Console: Firewalls → Create → inbound allow TCP 22, 80, 443 from anywhere → apply to `wythin-api`.
Or CLI:
```bash
...
hcloud firewall apply-to-resource web --type server --server wythin-api
```
...
The app's `DATABASE_URL` uses host `db` (the compose service name), not `localhost`. Confirm `docker-compose.yml` sets `DATABASE_URL=postgresql://wythin:...@db:5432/wythin` (it does, from Task 6).
...
mkdir -p backups
STAMP=$(date +%F-%H%M)
docker compose exec -T db pg_dump -U wythin wythin | gzip > "backups/wythin-$STAMP.sql.gz"
# Retain 7 days
...
Expected: a `wythin-<stamp>.sql.gz` file exists and is non-empty.
...
```bash
hcloud server create-image --type snapshot --description "wythin weekly" wythin-api
```
...
**Files:**
- Create: `~/ios/Wythin/App/ServerConfig.swift`
- Modify: `~/ios/Wythin/App/AppEnvironment.swift:74-75,107-108` (default server URL)
- Modify: `~/ios/Wythin/Sync/APIClient.swift:139-144` (`request` helper — add auth header)
- Modify: `~/ios/Wythin/Sync/SyncService.swift:60-70` (`connect` — add `?token=`)
...
In `~/ios/Wythin/App/AppEnvironment.swift`, replace both `"http://localhost:8000"` fallbacks (lines ~74-75 and ~107-108) with `ServerConfig.defaultBaseURLString`. Example at line 74-75:
...
In `~/ios/Wythin/Sync/APIClient.swift`, in the private `request(path:method:)` helper, add the auth header:
...
In `~/ios/Wythin/Sync/SyncService.swift`, in `connect(userID:)`, add the token query item before building the URL:
...
```bash
cd /Users/alexutkin
git add ios/Wythin/App/ServerConfig.swift ios/Wythin/App/AppEnvironment.swift \
        ios/Wythin/Sync/APIClient.swift ios/Wythin/Sync/SyncService.swift
