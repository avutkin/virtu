...
New file `ios/Wythin/Metrics/LiveStateTrendCompute.swift`, alongside the existing `HRVCompute.swift`/`RSACompute.swift`/etc. — pure computation, no I/O, matching that directory's existing convention.
...
## 3. iOS — wire types (`ios/Wythin/Sync/APIClient.swift`)
...
New file `ios/Wythin/UI/Live/LiveStateWidget.swift`:
...
Placed in `DayScrollView`'s `isToday` branch, in `ios/Wythin/UI/Live/LiveView.swift`, directly above the existing `CurrentStateCard`:
...
| Modify | `ios/Wythin/Sync/APIClient.swift` — `MetricTrendPayload`, `LiveStateInsightPayload`, `generateLiveStateInsight`, `InsightAPIClient` protocol addition, `MetricTrendPayload(from: MetricTrend)` builder |
| Create | `ios/Wythin/UI/Live/LiveStateWidget.swift` |
| Modify | `ios/Wythin/UI/Live/LiveView.swift` — insert `LiveStateWidget()` above `CurrentStateCard` in `DayScrollView`'s `isToday` branch |
