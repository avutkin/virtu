...
`ActivityLog` (`ios/Wythin/Models/ActivityLog.swift`) gains one field:
...
### Wire types (`ios/Wythin/Sync/APIClient.swift`)
...
At each of the three `entry.computeHRVWindows(context: ctx)` call sites in `ios/Wythin/UI/Actions/ActionsView.swift` (lines 309, 368, 383), immediately after the windows are computed, fire a background `Task`:
...
Hooks into the existing `isInForeground` `didSet` in `ios/Wythin/App/AppEnvironment.swift:46-61`, in the same branch that already calls `reloadRecentHistory()` on return-to-foreground:
...
| Modify | `ios/Wythin/Models/ActivityLog.swift` — add `insightText: String?` |
| Modify | `ios/Wythin/Sync/APIClient.swift` — add `InsightPayload`, `InsightResponse`, `generateInsight` |
| Create | `ios/Wythin/Sync/InsightGenerator.swift` (or similar) — `generate(for:client:context:)` |
| Modify | `ios/Wythin/UI/Actions/ActionsView.swift` — fire generation `Task` at 3 `computeHRVWindows` call sites; add new Insight section to the `ActivityDetailView` struct defined in this same file |
| Modify | `ios/Wythin/App/AppEnvironment.swift` — `retryPendingInsights()` call in `isInForeground.didSet` |
