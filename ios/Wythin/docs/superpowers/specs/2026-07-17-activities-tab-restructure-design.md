...
`ContentView` (in `WythinApp.swift`) observes it:
...
| Modify | `App/WythinApp.swift` — `AppTab.actions` → `.activities`, tab label, `ActionsView()` → `ActivitiesView()`, `.onChange(of: env.pendingTabRequest)` |
