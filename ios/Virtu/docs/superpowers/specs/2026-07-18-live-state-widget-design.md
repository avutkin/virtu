...
New file `ios/Virtu/Metrics/LiveStateTrendCompute.swift`, alongside the existing `HRVCompute.swift`/`RSACompute.swift`/etc. — pure computation, no I/O, matching that directory's existing convention.
...
## 3. iOS — wire types (`ios/Virtu/Sync/APIClient.swift`)
...
New file `ios/Virtu/UI/Live/LiveStateWidget.swift`:
...
Placed in `DayScrollView`'s `isToday` branch, in `ios/Virtu/UI/Live/LiveView.swift`, directly above the existing `CurrentStateCard`:
...
| Modify | `ios/Virtu/Sync/APIClient.swift` — `MetricTrendPayload`, `LiveStateInsightPayload`, `generateLiveStateInsight`, `InsightAPIClient` protocol addition, `MetricTrendPayload(from: MetricTrend)` builder |
| Create | `ios/Virtu/UI/Live/LiveStateWidget.swift` |
| Modify | `ios/Virtu/UI/Live/LiveView.swift` — insert `LiveStateWidget()` above `CurrentStateCard` in `DayScrollView`'s `isToday` branch |
