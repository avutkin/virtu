...
`ContentView` (in `VirtuApp.swift`) observes it:
...
| Modify | `App/VirtuApp.swift` — `AppTab.actions` → `.activities`, tab label, `ActionsView()` → `ActivitiesView()`, `.onChange(of: env.pendingTabRequest)` |
