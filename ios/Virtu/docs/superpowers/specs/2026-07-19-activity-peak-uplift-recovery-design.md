...
lives in `Metrics/` and is covered by `VirtuTests`, while views are not). It
...
### Tests — `VirtuTests/ActivityMetricStatsTests.swift`
...
| Modify | `Virtu.xcodeproj/project.pbxproj` — register the 2 new files |
