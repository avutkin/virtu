...
- Modify: `App/VirtuApp.swift:67-86`
...
In `App/VirtuApp.swift`, `ContentView.body` currently ends with:
...
```bash
cd /Users/alexutkin/ios && git add Virtu/App/AppEnvironment.swift Virtu/App/VirtuApp.swift
...
```bash
cd /Users/alexutkin/ios/Virtu/UI
...
In `Virtu.xcodeproj/project.pbxproj`, there are 4 places referencing the old path/name. Make these exact replacements:
...
```bash
cd /Users/alexutkin/ios && git add -A Virtu/UI/Activities Virtu/UI/Actions Virtu.xcodeproj/project.pbxproj
...
xcodebuild -project Virtu.xcodeproj -scheme Virtu -destination 'platform=iOS Simulator,name=iPhone 17 Pro' -derivedDataPath /tmp/jb-build build 2>&1 | tail -5
xcrun simctl install "iPhone 17 Pro" /tmp/jb-build/Build/Products/Debug-iphonesimulator/Virtu.app
BUNDLE_ID=$(/usr/libexec/PlistBuddy -c "Print :CFBundleIdentifier" /tmp/jb-build/Build/Products/Debug-iphonesimulator/Virtu.app/Info.plist)
...
```bash
cd /Users/alexutkin/ios && git add Virtu/UI/Activities/ActivitiesView.swift Virtu/App/VirtuApp.swift
...
```bash
cd /Users/alexutkin/ios && xcodebuild clean build -project Virtu.xcodeproj -scheme Virtu -destination 'platform=iOS Simulator,name=iPhone 17 Pro' 2>&1 | tail -40
```
Expected: `** BUILD SUCCEEDED **`
