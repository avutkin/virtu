...
**Tech Stack:** Swift 5 / SwiftUI, Swift Charts, XCTest (`VirtuTests`), Xcode project with explicit `PBXFileReference`/`PBXGroup` entries (new files registered in `project.pbxproj` by hand).

## Global Constraints

- Compute logic is unit-tested (`VirtuTests`); SwiftUI views are not — matches codebase convention. Verification: build success + the new compute tests passing, plus a manual/visual check where noted.
- Build command:
  ```bash
...
- Test command (whole suite):
  ```bash
  cd /Users/alexutkin/ios && xcodebuild test -project Virtu.xcodeproj -scheme Virtu -destination 'platform=iOS Simulator,name=iPhone 17 Pro' 2>&1 | grep -E "error:|failed \(|Executed"
  ```
...
- Create: `VirtuTests/ActivityMetricStatsTests.swift`
...
- [ ] **Step 2: Write `VirtuTests/ActivityMetricStatsTests.swift`**
...
- [ ] **Step 3: Register both files in `Virtu.xcodeproj/project.pbxproj`**
...
Addition F — add `FT07` to the `VirtuTests` group (`GTESTS`), after the `FT06 /* PayloadBuilderTests.swift */,` child:
Find:
```
...
```bash
cd /Users/alexutkin/ios && xcodebuild test -project Virtu.xcodeproj -scheme Virtu -destination 'platform=iOS Simulator,name=iPhone 17 Pro' -only-testing:VirtuTests/ActivityMetricStatsTests 2>&1 | grep -E "error:|failed \(|passed \(|Executed"
```
...
```bash
cd /Users/alexutkin/ios && git add Virtu/Metrics/ActivityMetricStats.swift VirtuTests/ActivityMetricStatsTests.swift Virtu.xcodeproj/project.pbxproj
git commit -m "feat(metrics): add ActivityMetricStats for benefit-aware peak/uplift/recovery"
```
(The test file lives at `ios/VirtuTests/ActivityMetricStatsTests.swift` — a sibling of `Virtu/`, alongside `PayloadBuilderTests.swift` etc. — NOT nested under `Virtu/`.)
...
```bash
cd /Users/alexutkin/ios && git add Virtu/UI/Design/MetricTile.swift
...
```bash
cd /Users/alexutkin/ios && xcodebuild test -project Virtu.xcodeproj -scheme Virtu -destination 'platform=iOS Simulator,name=iPhone 17 Pro' 2>&1 | grep -E "failed \(|Executed"
```
