# Virtu Backend — Hetzner Deployment Implementation Plan
...
Run: `cd ~/server && docker build -t virtu-api:local .`
Expected: build succeeds, ends with `naming to docker.io/library/virtu-api:local`.
...
- Delete: `~/server/virtu-api.service` (superseded by Compose)
...
services:
  db:
    ...
    healthcheck:
      test: ["CMD-SHELL", "pg_isready -U virtu"]
      interval: 10s
      timeout: 5s
      retries: 5
  ...
  api:
    ...
    environment:
      DATABASE_URL: postgresql://virtu:${POSTGRES_PASSWORD}@db:5432/virtu
...
Run: `cd /Users/alexutkin && git rm server/virtu-api.service`
...
Or CLI (`brew install hcloud`, then `hcloud context create virtu` with an API token):
```bash
hcloud server create --name virtu-api --type cax11 \
  --image ubuntu-24.04 --location nbg1 --ssh-key <your-key-name>
```

- [ ] **Step 2: Record the public IP**

Run: `hcloud server ip virtu-api` (or read it in the console).
...
Console: Firewalls → Create → inbound allow TCP 22, 80, 443 from anywhere → apply to `virtu-api`.
Or CLI:
```bash
...
hcloud firewall apply-to-resource web --type server --server virtu-api
```
...
The app's `DATABASE_URL` uses host `db` (the compose service name), not `localhost`. Confirm `docker-compose.yml` sets `DATABASE_URL=postgresql://virtu:...@db:5432/virtu` (it does, from Task 6).
...
mkdir -p backups
STAMP=$(date +%F-%H%M)
docker compose exec -T db pg_dump -U virtu virtu | gzip > "backups/virtu-$STAMP.sql.gz"
# Retain 7 days
...
Expected: a `virtu-<stamp>.sql.gz` file exists and is non-empty.
...
```bash
hcloud server create-image --type snapshot --description "virtu weekly" virtu-api
```
...
**Files:**
- Create: `~/ios/Virtu/App/ServerConfig.swift`
- Modify: `~/ios/Virtu/App/AppEnvironment.swift:74-75,107-108` (default server URL)
- Modify: `~/ios/Virtu/Sync/APIClient.swift:139-144` (`request` helper — add auth header)
- Modify: `~/ios/Virtu/Sync/SyncService.swift:60-70` (`connect` — add `?token=`)
...
In `~/ios/Virtu/App/AppEnvironment.swift`, replace both `"http://localhost:8000"` fallbacks (lines ~74-75 and ~107-108) with `ServerConfig.defaultBaseURLString`. Example at line 74-75:
...
In `~/ios/Virtu/Sync/APIClient.swift`, in the private `request(path:method:)` helper, add the auth header:
...
In `~/ios/Virtu/Sync/SyncService.swift`, in `connect(userID:)`, add the token query item before building the URL:
...
```bash
cd /Users/alexutkin
git add ios/Virtu/App/ServerConfig.swift ios/Virtu/App/AppEnvironment.swift \
        ios/Virtu/Sync/APIClient.swift ios/Virtu/Sync/SyncService.swift
