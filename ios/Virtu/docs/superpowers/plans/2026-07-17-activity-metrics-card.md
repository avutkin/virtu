...
- No unit-test target covers SwiftUI view/model files in this codebase (`VirtuTests` only covers BLE parsing and metrics compute logic) — verification for every task is **build success** (`xcodebuild build`) plus, where noted, a manual Simulator check.
...
```bash
cd /Users/alexutkin/ios && git add Virtu/Models/ActivityLog.swift
...
Write `ios/Virtu/UI/Design/MetricTile.swift`:
...
In `ios/Virtu/UI/Live/LiveView.swift`, find and delete this entire block (it currently sits directly after `MetricsTableView`, just before `// MARK: - Preview`):
...
In `Virtu.xcodeproj/project.pbxproj`, make these 4 additions (new unique IDs `F145`/`A145` — confirmed not already used by `grep -oE "F1[0-9]+|A1[0-9]+" Virtu.xcodeproj/project.pbxproj`, current highest is `F144`/`A144`):
...
```bash
cd /Users/alexutkin/ios && git add Virtu/UI/Design/MetricTile.swift Virtu/UI/Live/LiveView.swift Virtu.xcodeproj/project.pbxproj
...
Write `ios/Virtu/UI/Activities/ActivityMetricsCard.swift`:
...
In `Virtu.xcodeproj/project.pbxproj`, make these 4 additions (new IDs `F146`/`A146` — one past the `F145`/`A145` pair added in Task 2):
...
```bash
cd /Users/alexutkin/ios && git add Virtu/UI/Activities/ActivityMetricsCard.swift Virtu.xcodeproj/project.pbxproj
...
xcodebuild -project Virtu.xcodeproj -scheme Virtu -destination 'platform=iOS Simulator,name=iPhone 17 Pro' -derivedDataPath /tmp/jb-build-t2t4 build 2>&1 | tail -5
xcrun simctl install "iPhone 17 Pro" /tmp/jb-build-t2t4/Build/Products/Debug-iphonesimulator/Virtu.app
BUNDLE_ID=$(/usr/libexec/PlistBuddy -c "Print :CFBundleIdentifier" /tmp/jb-build-t2t4/Build/Products/Debug-iphonesimulator/Virtu.app/Info.plist)
