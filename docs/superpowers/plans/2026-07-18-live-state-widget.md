...
**Files:**
- Create: `ios/Virtu/Metrics/LiveStateTrendCompute.swift`
- Modify: `ios/Virtu/Models/MetricsHistoryPoint.swift`
- Test: `ios/VirtuTests/LiveStateTrendComputeTests.swift`
...
In `ios/Virtu/Models/MetricsHistoryPoint.swift`, add after the existing `init(from sample: HRVSample)` initializer (before the closing brace of the struct):
...
Create `ios/VirtuTests/LiveStateTrendComputeTests.swift`:
...
`LiveStateTrendCompute.swift` (Step 5 below) goes in the app target's `Metrics/` group; `LiveStateTrendComputeTests.swift` goes in the `VirtuTests` target. Follow the exact 4-location pattern already used for every other file in `ios/Virtu.xcodeproj/project.pbxproj` (`PBXBuildFile`, `PBXFileReference`, the group's `children` list, the target's `Sources` build phase `files` list) — search the file for an existing entry in the relevant group (e.g. `RSACompute.swift` for the app target, `InsightGeneratorTests.swift` for the test target) to find the exact 4 spots and copy the pattern with new unique identifiers.

- [ ] **Step 4: Run the tests to verify they fail**

Run: `xcodebuild test -project ios/Virtu.xcodeproj -scheme Virtu -destination 'platform=iOS Simulator,name=iPhone 17' -only-testing:VirtuTests/LiveStateTrendComputeTests CODE_SIGN_IDENTITY="" CODE_SIGNING_REQUIRED=NO 2>&1 | tail -30`
...
```bash
git add ios/Virtu/Metrics/LiveStateTrendCompute.swift ios/Virtu/Models/MetricsHistoryPoint.swift ios/VirtuTests/LiveStateTrendComputeTests.swift ios/Virtu.xcodeproj/project.pbxproj
...
**Files:**
- Modify: `ios/Virtu/Sync/APIClient.swift`
- Modify: `ios/VirtuTests/InsightGeneratorTests.swift`
- Test: `ios/VirtuTests/PayloadBuilderTests.swift`
...
In `ios/Virtu/Sync/APIClient.swift`, insert after the `InsightPayload` struct (after its closing brace, before `struct InsightResponse`):
...
In `ios/VirtuTests/InsightGeneratorTests.swift`, change:
...
Create `ios/VirtuTests/PayloadBuilderTests.swift`:
...
Register this file in the Xcode project's `VirtuTests` target, same 4-location `project.pbxproj` pattern as before.

- [ ] **Step 5: Run the tests to verify they fail**

Run: `xcodebuild test -project ios/Virtu.xcodeproj -scheme Virtu -destination 'platform=iOS Simulator,name=iPhone 17' -only-testing:VirtuTests/PayloadBuilderTests CODE_SIGN_IDENTITY="" CODE_SIGNING_REQUIRED=NO 2>&1 | tail -30`
...
Run: `xcodebuild test -project ios/Virtu.xcodeproj -scheme Virtu -destination 'platform=iOS Simulator,name=iPhone 17' -only-testing:VirtuTests/InsightGeneratorTests CODE_SIGN_IDENTITY="" CODE_SIGNING_REQUIRED=NO 2>&1 | tail -30`
...
```bash
git add ios/Virtu/Sync/APIClient.swift ios/VirtuTests/InsightGeneratorTests.swift ios/VirtuTests/PayloadBuilderTests.swift ios/Virtu.xcodeproj/project.pbxproj
...
**Files:**
- Create: `ios/Virtu/UI/Live/LiveStateWidget.swift`
- Modify: `ios/Virtu/UI/Live/LiveView.swift`

**Interfaces:**
- Consumes: `LiveStateTrendCompute.summarize` (Task 2), `MetricsQualityFilter.filter` (existing, `ios/Virtu/Models/MetricsHistoryPoint.swift`), `LiveStateInsightPayload.init(windowMinutes:trends:)` and `APIClient.generateLiveStateInsight` (Task 3), `env.tickHistory`, `env.ble.state` (`BLEState`, existing, `ios/Virtu/BLE/BLEService.swift` — `.connected(name: String)` is an associated-value case, requires `if case` pattern matching, not `==`), `env.sync.client` (existing), `.cardStyle()`/`Theme.monoBody`/`Theme.dim`/`Theme.text` (existing, `ios/Virtu/UI/Design/Theme.swift`).
...
In `ios/Virtu/UI/Live/LiveView.swift`, change:
...
Run: `xcodebuild build -project ios/Virtu.xcodeproj -scheme Virtu -destination 'platform=iOS Simulator,name=iPhone 17' CODE_SIGN_IDENTITY="" CODE_SIGNING_REQUIRED=NO 2>&1 | tail -20`
Expected: `** BUILD SUCCEEDED **`

- [ ] **Step 5: Commit**

```bash
git add ios/Virtu/UI/Live/LiveStateWidget.swift ios/Virtu/UI/Live/LiveView.swift ios/Virtu.xcodeproj/project.pbxproj
...
export DATABASE_URL="postgresql://postgres:postgres@localhost:5432/virtu"
...
Build and run the `Virtu` scheme on a physical device or simulator, with `serverURL` pointing at this machine's local IP on port 8000 (a simulator can reach `localhost:8000` directly since the Mac is the host; a physical device needs the Mac's LAN IP — set via Settings if the app exposes a server URL field, or `UserDefaults.standard.set(...)` for `"serverURL"` during a debug session). Pair a Polar H10 (simulator can't — this step needs a physical device with real BLE hardware, or previously-recorded HRV samples already in the SwiftData store from an earlier session so `env.tickHistory` has ≥60 quality-passing points once merged in on foreground).
