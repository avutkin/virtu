...
- Modify: `ios/JustBreathe/Models/MetricsHistoryPoint.swift`
- Test: `ios/JustBreatheTests/LiveStateTrendComputeTests.swift`
...
In `ios/JustBreathe/Models/MetricsHistoryPoint.swift`, add after the existing `init(from sample: HRVSample)` initializer (before the closing brace of the struct):
...
`LiveStateTrendCompute.swift` (Step 5 below) goes in the app target's `Metrics/` group; `LiveStateTrendComputeTests.swift` goes in the `JustBreatheTests` target. Follow the exact 4-location pattern already used for every other file in `ios/JustBreathe.xcodeproj/project.pbxproj` (`PBXBuildFile`, `PBXFileReference`, the group's `children` list, the target's `Sources` build phase `files` list) — search the file for an existing entry in the relevant group (e.g. `RSACompute.swift` for the app target, `InsightGeneratorTests.swift` for the test target) to find the exact 4 spots and copy the pattern with new unique identifiers.

- [ ] **Step 4: Run the tests to verify they fail**

Run: `xcodebuild test -project ios/JustBreathe.xcodeproj -scheme JustBreathe -destination 'platform=iOS Simulator,name=iPhone 17' -only-testing:JustBreatheTests/LiveStateTrendComputeTests CODE_SIGN_IDENTITY="" CODE_SIGNING_REQUIRED=NO 2>&1 | tail -30`
...
```bash
git add ios/JustBreathe/Metrics/LiveStateTrendCompute.swift ios/JustBreathe/Models/MetricsHistoryPoint.swift ios/JustBreatheTests/LiveStateTrendComputeTests.swift ios/JustBreathe.xcodeproj/project.pbxproj
...
**Files:**
- Modify: `ios/JustBreathe/Sync/APIClient.swift`
- Modify: `ios/JustBreatheTests/InsightGeneratorTests.swift`
- Test: `ios/JustBreatheTests/PayloadBuilderTests.swift`
...
In `ios/JustBreathe/Sync/APIClient.swift`, insert after the `InsightPayload` struct (after its closing brace, before `struct InsightResponse`):
...
In `ios/JustBreatheTests/InsightGeneratorTests.swift`, change:
...
Create `ios/JustBreatheTests/PayloadBuilderTests.swift`:
...
Register this file in the Xcode project's `JustBreatheTests` target, same 4-location `project.pbxproj` pattern as before.

- [ ] **Step 5: Run the tests to verify they fail**

Run: `xcodebuild test -project ios/JustBreathe.xcodeproj -scheme JustBreathe -destination 'platform=iOS Simulator,name=iPhone 17' -only-testing:JustBreatheTests/PayloadBuilderTests CODE_SIGN_IDENTITY="" CODE_SIGNING_REQUIRED=NO 2>&1 | tail -30`
...
Run: `xcodebuild test -project ios/JustBreathe.xcodeproj -scheme JustBreathe -destination 'platform=iOS Simulator,name=iPhone 17' -only-testing:JustBreatheTests/InsightGeneratorTests CODE_SIGN_IDENTITY="" CODE_SIGNING_REQUIRED=NO 2>&1 | tail -30`
...
```bash
git add ios/JustBreathe/Sync/APIClient.swift ios/JustBreatheTests/InsightGeneratorTests.swift ios/JustBreatheTests/PayloadBuilderTests.swift ios/JustBreathe.xcodeproj/project.pbxproj
...
**Interfaces:**
- Consumes: `LiveStateTrendCompute.summarize` (Task 2), `MetricsQualityFilter.filter` (existing, `ios/JustBreathe/Models/MetricsHistoryPoint.swift`), `LiveStateInsightPayload.init(windowMinutes:trends:)` and `APIClient.generateLiveStateInsight` (Task 3), `env.tickHistory`, `env.ble.state` (`BLEState`, existing, `ios/JustBreathe/BLE/BLEService.swift` — `.connected(name: String)` is an associated-value case, requires `if case` pattern matching, not `==`), `env.sync.client` (existing), `.cardStyle()`/`Theme.monoBody`/`Theme.dim`/`Theme.text` (existing, `ios/JustBreathe/UI/Design/Theme.swift`).
...
In `ios/JustBreathe/UI/Live/LiveView.swift`, change:
...
Run: `xcodebuild build -project ios/JustBreathe.xcodeproj -scheme JustBreathe -destination 'platform=iOS Simulator,name=iPhone 17' CODE_SIGN_IDENTITY="" CODE_SIGNING_REQUIRED=NO 2>&1 | tail -20`
Expected: `** BUILD SUCCEEDED **`

- [ ] **Step 5: Commit**

```bash
git add ios/JustBreathe/UI/Live/LiveStateWidget.swift ios/JustBreathe/UI/Live/LiveView.swift ios/JustBreathe.xcodeproj/project.pbxproj
...
export DATABASE_URL="postgresql://postgres:postgres@localhost:5432/justbreathe"
...
Build and run the `JustBreathe` scheme on a physical device or simulator, with `serverURL` pointing at this machine's local IP on port 8000 (a simulator can reach `localhost:8000` directly since the Mac is the host; a physical device needs the Mac's LAN IP — set via Settings if the app exposes a server URL field, or `UserDefaults.standard.set(...)` for `"serverURL"` during a debug session). Pair a Polar H10 (simulator can't — this step needs a physical device with real BLE hardware, or previously-recorded HRV samples already in the SwiftData store from an earlier session so `env.tickHistory` has ≥60 quality-passing points once merged in on foreground).
