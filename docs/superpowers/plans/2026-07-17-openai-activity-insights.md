...
- Generation is fire-and-forget from the 3 `computeHRVWindows` call sites in `ios/Virtu/UI/Actions/ActionsView.swift` (lines 309, 368, 383) — never blocks activity logging (spec §3).
...
**Interfaces:**
- Consumes: `ActivityLog` fields (`activityType`, `activitySubtype`, `duration`, `beforeHR`/`duringHR`/`afterHR`, `beforeRSA`/`duringRSA`/`afterRSA`, `beforeSDNN`/`duringSDNN`/`afterSDNN`, `beforeLFHF`/`duringLFHF`/`afterLFHF`) — all already defined in `ios/Virtu/Models/ActivityLog.swift`.
...
In `ios/Virtu/Sync/APIClient.swift`, insert after the `TickPayload` struct (after line 38, before `struct ServerSession`):
...
Run: `xcodebuild build -project ios/Virtu.xcodeproj -scheme Virtu -destination 'platform=iOS Simulator,name=iPhone 17' | tail -20`
Expected: `** BUILD SUCCEEDED **`

- [ ] **Step 5: Commit**

```bash
git add ios/Virtu/Sync/APIClient.swift
...
**Files:**
- Modify: `ios/Virtu/Models/ActivityLog.swift`
- Create: `ios/Virtu/Sync/InsightGenerator.swift`
- Test: `ios/VirtuTests/InsightGeneratorTests.swift`
...
This mirrors the existing `SessionUploader` actor (`ios/Virtu/Sync/SessionUploader.swift`) — same shape, same pattern, for unsynced insights instead of unsynced sessions.

- [ ] **Step 1: Add the field to `ActivityLog`**

In `ios/Virtu/Models/ActivityLog.swift`, change:
...
Create `ios/VirtuTests/InsightGeneratorTests.swift`:
...
Run: `xcodebuild test -project ios/Virtu.xcodeproj -scheme Virtu -destination 'platform=iOS Simulator,name=iPhone 17' -only-testing:VirtuTests/InsightGeneratorTests 2>&1 | tail -30`
...
```bash
git add ios/Virtu/Models/ActivityLog.swift ios/Virtu/Sync/InsightGenerator.swift ios/VirtuTests/InsightGeneratorTests.swift
...
**Files:**
- Modify: `ios/Virtu/UI/Actions/ActionsView.swift`
...
In `ios/Virtu/UI/Actions/ActionsView.swift`, change the `.edit` case (around line 307):
...
```bash
git add ios/Virtu/UI/Actions/ActionsView.swift
...
**Files:**
- Modify: `ios/Virtu/App/AppEnvironment.swift`
...
In `ios/Virtu/App/AppEnvironment.swift`, change:
...
```bash
git add ios/Virtu/App/AppEnvironment.swift
...
**Interfaces:**
- Consumes: `entry.insightText` (from Task 3), `Theme.monoLabel`/`Theme.monoBody`/`Theme.dim`/`Theme.text` and `.cardStyle()` (existing, `ios/Virtu/UI/Design/Theme.swift`).

- [ ] **Step 1: Add the Insight section**

In `ios/Virtu/UI/Actions/ActionsView.swift`, inside `ActivityDetailView.body`, change:
...
Build and run the `Virtu` scheme in a simulator (no BLE hardware needed — the simulator can't pair a Polar H10, so use the "Log Past" flow, which is `isManual` and goes through the same `computeHRVWindows` + insight-generation path as a live-tracked activity). In the Activities/Actions tab, tap **LOG PAST**, fill in an activity type and a start/end time, save.
