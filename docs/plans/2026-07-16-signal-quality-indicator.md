...
- `ios/Virtu.xcodeproj/project.pbxproj` uses manual `PBXFileReference`/`PBXBuildFile` entries — **no** synchronized file groups. Every new `.swift` file must be registered by hand (see Critical Context below) or the build won't see it.
...
- **Xcode project file** is at `ios/Virtu.xcodeproj/project.pbxproj`. New files need four edits each: a `PBXBuildFile` entry, a `PBXFileReference` entry, an entry in the relevant `PBXGroup`'s `children`, and an entry in the relevant target's `PBXSourcesBuildPhase` `files` list. Next available tokens: **F143/A143** (main target), **FT03/AT03** (test target).
- **Tests** live in `ios/VirtuTests/`, use `import XCTest` + `@testable import Virtu`, `XCTestCase` subclasses. See `ios/VirtuTests/MetricsTests.swift` for the exact style used in this project.
- **Test runner:** `cd /Users/alexutkin/ios && xcodebuild test -project Virtu.xcodeproj -scheme Virtu -sdk iphonesimulator -destination 'platform=iOS Simulator,name=iPhone 17' -only-testing:VirtuTests/ECGQualityComputeTests`
- **Full build check:** `cd /Users/alexutkin/ios && xcodebuild -project Virtu.xcodeproj -scheme Virtu -sdk iphonesimulator -destination 'generic/platform=iOS Simulator' -configuration Debug build`
...
**Files:**
- Create: `ios/Virtu/Metrics/ECGQualityCompute.swift`
- Create: `ios/VirtuTests/ECGQualityComputeTests.swift`
- Modify: `ios/Virtu.xcodeproj/project.pbxproj`
...
In `ios/Virtu.xcodeproj/project.pbxproj`, make these edits:
...
```
		...
		FAPP /* Virtu.app */ = {isa = PBXFileReference; explicitFileType = wrapper.application; includeInIndex = 0; path = Virtu.app; sourceTree = BUILT_PRODUCTS_DIR; };
...
**d)** In the `GTESTS /* VirtuTests */` group's `children`, find:
```
...
Run:
```bash
cd /Users/alexutkin/ios && xcodebuild test -project Virtu.xcodeproj -scheme Virtu -sdk iphonesimulator -destination 'platform=iOS Simulator,name=iPhone 17' -only-testing:VirtuTests/ECGQualityComputeTests
```
...
```bash
git add ios/Virtu/Metrics/ECGQualityCompute.swift \
        ios/VirtuTests/ECGQualityComputeTests.swift \
        ios/Virtu.xcodeproj/project.pbxproj
...
**Files:**
- Modify: `ios/Virtu/Metrics/ECGQualityCompute.swift`
- Modify: `ios/VirtuTests/ECGQualityComputeTests.swift`
...
Add to `ios/VirtuTests/ECGQualityComputeTests.swift` (inside the `ECGQualityComputeTests` class):
...
In `ios/Virtu/Metrics/ECGQualityCompute.swift`, replace:
```swift
        guard stddev >= flatlineStddevThreshold else {
            return ECGQualityResult(tier: .poor, reason: "lead-off")
        }
...
```bash
git add ios/Virtu/Metrics/ECGQualityCompute.swift ios/VirtuTests/ECGQualityComputeTests.swift
...
Add to `ios/VirtuTests/ECGQualityComputeTests.swift`:
...
In `ios/Virtu/Metrics/ECGQualityCompute.swift`, add these two static functions inside `enum ECGQualityCompute` (after `compute(ecg:)`):
...
**Files:**
- Modify: `ios/Virtu/Metrics/MetricsEngine.swift`
- Modify: `ios/VirtuTests/MetricsTests.swift`
- Modify: `ios/Virtu/UI/Live/LiveView.swift` (two pre-existing `MetricsTick` construction sites need the new field)
- Modify: `ios/Virtu/UI/Live/MetricsChartsView.swift` (one pre-existing `MetricsTick` construction site needs the new field)
...
Add to `ios/VirtuTests/MetricsTests.swift` (inside `MetricsTests`, e.g. after `testCBIRange`):
...
Run:
```bash
cd /Users/alexutkin/ios && xcodebuild test -project Virtu.xcodeproj -scheme Virtu -sdk iphonesimulator -destination 'platform=iOS Simulator,name=iPhone 17' -only-testing:VirtuTests/MetricsTests/testMetricsEngineComputesECGQuality
```
Expected: **BUILD FAILED** — `value of type 'MetricsTick' has no member 'ecgQuality'`.

- [ ] **Step 3: Add the field and wire the call**

In `ios/Virtu/Metrics/MetricsEngine.swift`, add the new field to `MetricsTick` (after the existing `signalQuality` field):
```swift
    // Signal quality
...
Adding a new non-defaulted field to `MetricsTick` makes it a required argument everywhere the struct is built. Three other places build one directly (`grep -rn "MetricsTick(" ios/Virtu` to confirm no others exist beyond these plus `MetricsEngine.swift`):

**a)** In `ios/Virtu/UI/Live/LiveView.swift`, `dayAverageTick(from:)` builds a `MetricsTick` averaged from `MetricsHistoryPoint`s, which don't carry ECG-quality data (it's live-only, per design) — pass `nil`. Replace:
```swift
...
**c)** In `ios/Virtu/UI/Live/MetricsChartsView.swift`, `mockHistory()` (another preview mock) builds a `MetricsTick` per synthetic data point — same reasoning as (a), pass `nil`. Replace:
```swift
...
```bash
cd /Users/alexutkin/ios && xcodebuild test -project Virtu.xcodeproj -scheme Virtu -sdk iphonesimulator -destination 'platform=iOS Simulator,name=iPhone 17'
cd /Users/alexutkin/ios && xcodebuild -project Virtu.xcodeproj -scheme Virtu -sdk iphonesimulator -destination 'generic/platform=iOS Simulator' -configuration Debug build
```
...
```bash
git add ios/Virtu/Metrics/MetricsEngine.swift ios/VirtuTests/MetricsTests.swift \
        ios/Virtu/UI/Live/LiveView.swift ios/Virtu/UI/Live/MetricsChartsView.swift
...
**Files:**
- Modify: `ios/Virtu/UI/Design/BLENavButton.swift`
- Modify: `ios/Virtu/UI/Live/LiveView.swift`
...
In `ios/Virtu/UI/Design/BLENavButton.swift`, replace:
```swift
...
In `ios/Virtu/UI/Live/LiveView.swift`, add this computed property to `LiveView` (after `private func goForward() { ... }`):
```swift
...
```bash
git add ios/Virtu/UI/Design/BLENavButton.swift ios/Virtu/UI/Live/LiveView.swift
...
```bash
git add ios/Virtu/UI/Live/LiveView.swift
